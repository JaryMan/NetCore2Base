...
namespace MeiMing.Framework.Provider
{
    ...
    public static class GoogleTranslate
    {
        ...
        /// <param name="translateFromLang"></param>
        /// <param name="translateToLang"></param>
        /// <returns></returns>
        public static String Translate(this String content, String translateFromLang, String translateToLang)
        {
            var stringBuilder = new StringBuilder("http://translate.google.cn/translate_a/t?client=f&sc=2&ie=UTF-8&oe=UTF-8&hl=zh-CN&oc=1&otf=2&ssel=0&tsel=0");
            stringBuilder.Append("&sl=");
            stringBuilder.Append(translateFromLang);
            stringBuilder.Append("&tl=");
            stringBuilder.Append(translateToLang);
            var tmpContent = String.Concat("q=", HttpUtility.UrlEncode(content));
            var strHtml = String.Empty;
            try
            {
                strHtml = GetUrlData(stringBuilder.ToString(), tmpContent, Encoding.UTF8);
            }
            catch (Exception)
            {
                while (true)
            ...
            var jsonObj = strHtml.FromJson<JsonObject>();
            strHtml = GetUrlData(stringBuilder.ToString(), tmpContent, Encoding.UTF8);
            jsonObj = strHtml.FromJson<JsonObject>();
            var translateStringBuilder = new StringBuilder(content.Length);
            foreach (var arrayObject in jsonObj.ArrayObjects("sentences"))
            {
                translateStringBuilder.Append(arrayObject["trans"]);
            }
        ...
        /// <returns></returns>
        private static string GetUrlData(string url, string postString, Encoding code)
        {
            //if (postString.Length < 100) url = String.Concat(url,"&", postString);
            var request = (HttpWebRequest)WebRequest.Create(url);
            request.Timeout = 60 * 1000 * 120;
            request.ReadWriteTimeout = 60 * 1000 * 120;
            request.AllowAutoRedirect = false;
            request.UserAgent = ChangeUserAgent();
            request.Referer = "http://translate.google.cn/";
            ...
            //request.Headers.Add("Cookie", "NID=67=YgycroO3cdl4PxWPm1pufIJ5-p5aygaRzcHCRlBa42KpT5VIF6gGvN1wBMMAEIRKtlNKwdOzF4mzhCMqIlAUTxZh7di6eSI2M_fr7CtxCBtrxPc8LCtpcfIUoFR5dYFO; PREF=ID=ee5ca0b4e064354e:U=761910997f0d578f:NW=1:TM=1305822879:LM=1395296974:S=qlwnXgqH3frcZ78W; _ga=GA1.3.1132927801.1384797188");

            if (postString.Length > 0)
            {
                request.ContentType = "application/x-www-form-urlencoded";
                request.Method = "POST";
                var postData = code.GetBytes(postString);
            ...
            var reader = new StreamReader(stream, code);
        ...
        /// </summary>
        /// <returns></returns>
        public static string ChangeUserAgent()
        {
            #region userAgents
            string[] userAgents = {
