...
namespace MeiMing.Framework.Provider
{
    public class QrCodeGenerator
    {
        ...
        public QrCode CreateQrCode(string plainText, ECCLevel eccLevel, bool utf8Bom = false)
        {
            ...
            while (bitString.Length < dataLength) bitString += "1110110000010001";
            ...
            for (var i = 0; i < eccInfo.BlocksInGroup1; i++)
            {
                ...
                codeWordWithEcc.Add(new CodewordBlock {BitString = bitStr, BlockNumber = i + 1, GroupNumber = 1, CodeWords = BinaryStringToBitBlockList(bitStr), ECCWords = CalculateEccWords(bitStr, eccInfo)});
            }
            ...
            for (var i = 0; i < eccInfo.BlocksInGroup2; i++)
            {
                ...
                codeWordWithEcc.Add(new CodewordBlock {BitString = bitStr, BlockNumber = i + 1, GroupNumber = 2, CodeWords = BinaryStringToBitBlockList(bitStr), ECCWords = CalculateEccWords(bitStr, eccInfo)});
            }
            ...
            {
                foreach (var codeBlock in codeWordWithEcc) if (codeBlock.CodeWords.Count > i) interleavedWordsSb.Append(codeBlock.CodeWords[i]);
            }


            for (var i = 0; i < eccInfo.ECCPerBlock; i++)
            {
                foreach (var codeBlock in codeWordWithEcc) if (codeBlock.ECCWords.Count > i) interleavedWordsSb.Append(codeBlock.ECCWords[i]);
            }
        ...
        private string GetFormatString(ECCLevel level, int maskVersion)
        {
            ...
            while (fStrEcc.Length > 10)
            {
                ...
                for (var i = 0; i < fStrEcc.Length; i++) sb.Append((Convert.ToInt32(fStrEcc[i]) ^ Convert.ToInt32(generator[i])).ToString());
            ...
            for (var i = 0; i < fStr.Length; i++) sbMask.Append((Convert.ToInt32(fStr[i]) ^ Convert.ToInt32(fStrMask[i])).ToString());
        ...
        private string GetVersionString(int version)
        {
            ...
            while (vStrEcc.Length > 12)
            {
                ...
                for (var i = 0; i < vStrEcc.Length; i++) sb.Append((Convert.ToInt32(vStrEcc[i]) ^ Convert.ToInt32(generator[i])).ToString());
        ...
        private List<string> CalculateEccWords(string bitString, ECCInfo eccInfo)
        {
            ...
            for (var i = 0; i < messagePolynom.PolyItems.Count; i++) messagePolynom.PolyItems[i] = new PolynomItem {Coefficient = messagePolynom.PolyItems[i].Coefficient, Exponent = messagePolynom.PolyItems[i].Exponent + eccWords};

            var genLeadtermFactor = messagePolynom.PolyItems[0].Exponent - generatorPolynom.PolyItems[0].Exponent;
            for (var i = 0; i < generatorPolynom.PolyItems.Count; i++) generatorPolynom.PolyItems[i] = new PolynomItem {Coefficient = generatorPolynom.PolyItems[i].Coefficient, Exponent = generatorPolynom.PolyItems[i].Exponent + genLeadtermFactor};
        ...
        private Polynom ConvertToAlphaNotation(Polynom poly)
        {
            ...
            for (var i = 0; i < poly.PolyItems.Count; i++) newPoly.PolyItems.Add(new PolynomItem {Coefficient = (poly.PolyItems[i].Coefficient != 0 ? GetAlphaExpFromIntVal(poly.PolyItems[i].Coefficient) : 0), Exponent = poly.PolyItems[i].Exponent});
            return newPoly;
        }
        ...
        {
            var newPoly = new Polynom();
            for (var i = 0; i < poly.PolyItems.Count; i++) newPoly.PolyItems.Add(new PolynomItem {Coefficient = GetIntValFromAlphaExp(poly.PolyItems[i].Coefficient), Exponent = poly.PolyItems[i].Exponent});
            return newPoly;
        }
        ...
        {
            if (plainText.All(c => "0123456789".Contains(c))) return EncodingMode.Numeric;
            if (plainText.All(c => _alphanumEncTable.Contains(c))) return EncodingMode.Alphanumeric;
        ...
        private Polynom CalculateMessagePolynom(string bitString)
        {
            ...
            {
                messagePol.PolyItems.Add(new PolynomItem {Coefficient = BinToDec(bitString.Substring(0, 8)), Exponent = i});
        ...
        private Polynom CalculateGeneratorPolynom(int numEccWords)
        {
            ...
            generatorPolynom.PolyItems.AddRange(new[] {new PolynomItem {Coefficient = 0, Exponent = 1}, new PolynomItem {Coefficient = 0, Exponent = 0}});
            for (var i = 1; i <= numEccWords - 1; i++)
            {
                var multiplierPolynom = new Polynom();
                multiplierPolynom.PolyItems.AddRange(new[] {new PolynomItem {Coefficient = 0, Exponent = 1}, new PolynomItem {Coefficient = i, Exponent = 0}});
        ...
        {
            return bitString.ToList().Select((x, i) => new {Index = i, Value = x}).GroupBy(x => x.Index / 8).Select(x => String.Join("", x.Select(v => v.Value.ToString()).ToArray())).ToList();
        }
        ...
        private int GetCountIndicatorLength(int version, EncodingMode encMode)
        {
            if (version < 10)
            {
                if (encMode.Equals(EncodingMode.Numeric)) return 10;
                if (encMode.Equals(EncodingMode.Alphanumeric)) return 9;
                return 8;
            }
            if (version < 27)
            {
                if (encMode.Equals(EncodingMode.Numeric)) return 12;
                if (encMode.Equals(EncodingMode.Alphanumeric)) return 11;
                if (encMode.Equals(EncodingMode.Byte)) return 16;
                return 10;
            }
            if (encMode.Equals(EncodingMode.Numeric)) return 14;
            if (encMode.Equals(EncodingMode.Alphanumeric)) return 13;
            if (encMode.Equals(EncodingMode.Byte)) return 16;
            return 12;
        }
        ...
        {
            if (encMode.Equals(EncodingMode.Numeric)) return PlainTextToBinaryNumeric(plainText);
            if (encMode.Equals(EncodingMode.Alphanumeric)) return PlainTextToBinaryAlphanumeric(plainText);
            if (encMode.Equals(EncodingMode.Byte)) return PlainTextToBinaryByte(plainText, utf8Bom);
            return string.Empty;
        }
        ...
        private string PlainTextToBinaryByte(string plainText, bool utf8Bom)
        {
            ...
            if (IsValidISO(plainText)) codeBytes = Encoding.GetEncoding("ISO-8859-1").GetBytes(plainText);
            else codeBytes = utf8Bom ? Encoding.UTF8.GetPreamble().Concat(Encoding.UTF8.GetBytes(plainText)).ToArray() : Encoding.UTF8.GetBytes(plainText);

            foreach (var b in codeBytes) codeText += DecToBin(b, 8);
        ...
        private void CreateAlphanumEncDict()
        {
            ...
            _alphanumEncTable.ToList().Select((x, i) => new {Chr = x, Index = i}).ToList().ForEach(x => _alphanumEncDict.Add(x.Chr, x.Index));
        }
        ...
        private void CreateAlignmentPatternTable()
        {
            ...
            for (var i = 0; i < (7 * 40); i = i + 7)
            {
                ...
                _alignmentPatternTable.Add(new AlignmentPattern {Version = (i + 7) / 7, PatternPositions = points});
            }
        }
        ...
        private void CreateCapacityTable()
        {
            ...
            {
                _capacityTable.Add(new VersionInfo {Version = (i + 16) / 16, Details = new List<VersionInfoDetails> {new VersionInfoDetails {ErrorCorrectionLevel = ECCLevel.L, CapacityDict = new Dictionary<EncodingMode, int> {{EncodingMode.Numeric, _capacityBaseValues[i]}, {EncodingMode.Alphanumeric, _capacityBaseValues[i + 1]}, {EncodingMode.Byte, _capacityBaseValues[i + 2]}, {EncodingMode.Kanji, _capacityBaseValues[i + 3]}}}, new VersionInfoDetails {ErrorCorrectionLevel = ECCLevel.M, CapacityDict = new Dictionary<EncodingMode, int> {{EncodingMode.Numeric, _capacityBaseValues[i + 4]}, {EncodingMode.Alphanumeric, _capacityBaseValues[i + 5]}, {EncodingMode.Byte, _capacityBaseValues[i + 6]}, {EncodingMode.Kanji, _capacityBaseValues[i + 7]}}}, new VersionInfoDetails {ErrorCorrectionLevel = ECCLevel.Q, CapacityDict = new Dictionary<EncodingMode, int> {{EncodingMode.Numeric, _capacityBaseValues[i + 8]}, {EncodingMode.Alphanumeric, _capacityBaseValues[i + 9]}, {EncodingMode.Byte, _capacityBaseValues[i + 10]}, {EncodingMode.Kanji, _capacityBaseValues[i + 11]}}}, new VersionInfoDetails {ErrorCorrectionLevel = ECCLevel.H, CapacityDict = new Dictionary<EncodingMode, int> {{EncodingMode.Numeric, _capacityBaseValues[i + 12]}, {EncodingMode.Alphanumeric, _capacityBaseValues[i + 13]}, {EncodingMode.Byte, _capacityBaseValues[i + 14]}, {EncodingMode.Kanji, _capacityBaseValues[i + 15]}}}}});
            }
        }
        ...
        private void CreateAntilogTable()
        {
            ...
            for (var i = 0; i < 256; i++)
            {
                ...
                _galoisField.Add(new Antilog {ExponentAlpha = i, IntegerValue = gfItem});
            }
        }
    }
}